...
export const TASK_PCI_GUARDRAILS: GuardrailRule[] = [
  ...
  },
  {
    ...
    rule: "After task completion, students may ask follow-up questions about the task and the evaluation of their answers. During PCI setup, ask the tutor how such follow-ups should be answered (e.g., whether to reveal the answer, how much explanation to give, whether to allow retries, how to handle off-topic questions). If the tutor does not specify, the AI must use its best effort grounded in the PCI and the student's own answers. Restrict follow-up responses to the task, its evaluation, and directly related previous tasks; do not drift into unrelated conversation.",
    enforcement: ['prompt'],
  },
  {
    id: 'TASK-26',
    title: 'Language Preference Check',
    rule: 'Immediately after presenting the initial task summary, ask the tutor whether they would like to continue the PCI setup in that same language or switch to a different language. The PCI policy can be completed in any language the tutor chooses. Do not force a language; confirm the tutor’s preference before asking marking or policy questions.',
    enforcement: ['prompt'],
  },
  {
    id: 'TASK-27',
    title: 'Homework Folder Recommendations',
    rule: 'Before finalizing a task PCI, inspect the homework folder of the lesson that contains the task. Identify any TASKS or ASSESSMENTS stored there, and read their PCI instructions to determine each item’s instructional scope and intent. Ask the tutor whether any of those homework items should be recommended to students after they complete the current task. Base recommendations on how the student responds and on the instructional scope of each homework item (for example, suggest a remedial homework task if the student performed poorly and the homework item covers the same skill at a lower level, or an extension if they did well). Explain why each homework item is relevant by referencing its instructional scope and intent. Capture the tutor-approved recommendations in the PCI. If the homework folder is empty, say so plainly and do not invent homework items.',
    enforcement: ['prompt', 'validator'],
  },
]
