...
export function autoGradeDmi(
  ...
  if (gradable.length === 0) {
    return { score: null, questionResults: null, gradable: 0, correct: 0, needsReview: 0 }
  }
  ...
  for (const item of gradable) {
    ...
    questionResults.push({
      questionId: item.id,
      correct: matched,
      pointsEarned: matched ? POINTS_PER_ITEM : 0,
      pointsMax: review ? 0 : POINTS_PER_ITEM,
  ...
  return {
    score: counted > 0 ? Math.round((correct / counted) * 100) : null,
    questionResults,
