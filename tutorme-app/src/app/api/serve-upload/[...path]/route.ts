...
export const GET = withAuth(async (request: NextRequest, session: any, context: any) => {
  try {
    ...
    try {
      ...
      if (isGcsConfigured()) {
        ...
        if (buf) {
          ...
          return new NextResponse(new Uint8Array(buf), {
            headers: {
              'Content-Type': getContentType(gcsRelativePath),
      ...
      console.warn('[serve-upload] File not found in local or GCS storage:', gcsRelativePath)
    } catch (gcsError) {
      console.error('[serve-upload] GCS resolution failed for:', gcsRelativePath, gcsError)
    }
